...
globle void MVSlotInsertCommand(
  ...
  DATA_OBJECT *result)
  {
   ...
   if (sp == NULL)
     return;
   AssignSlotToDataObject(&oldseg,sp);
   if (InsertMultiValueField(theEnv,&newseg,&oldseg,index,&newval,"slot-insert$") == FALSE)
     return;
   arg.type = MULTIFIELD;
   arg.value = (void *) &newseg;
   arg.nextArg = NULL;
...
globle BOOLEAN DirectMVInsertCommand(
  void *theEnv)
  {
   ...
   if (sp == NULL)
     return(FALSE);
   AssignSlotToDataObject(&oldseg,sp);
   if (InsertMultiValueField(theEnv,&newseg,&oldseg,index,&newval,"direct-slot-insert$")
          == FALSE)
     return(FALSE);

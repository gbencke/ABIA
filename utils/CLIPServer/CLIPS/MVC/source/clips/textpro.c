...
int TextLookupFetch(void *,char *,char *,int);
...
static struct entries *AllocateEntryNode(void *,FILE *,char *,char *,char *,int,int);
static int AttachLeaf(void *,struct lists *,struct entries *,FILE *,char *,char *,int,int);
      ...
      if ((str[0] != LIT_DELIM) || (str[1] != LIT_DELIM))
        ...
        if (findstr(str,EDELIM) >= 0)
        ...
        else if (findstr(str,BDELIM) >= 0)
          {
           ...
           if ((enode=AllocateEntryNode(theEnv,fp,file,str,errbuf,bufsize,line_ct))==NULL)
             return(0);

           /*=================================*/
           /*Store new entry node in the tree */
           /*=================================*/
           if (AttachLeaf(theEnv,lnode,enode,fp,file,errbuf,bufsize,line_ct) == FALSE)
             return(0);
          }
        }
     }
...
static FILE *GetCurrentMenu(
  ...
  int *status)
  {
   ...
   if ((fp = GenOpen(file,OPEN_READ)) == NULL)
     {
      *status = NO_FILE;
      return(NULL);
     }
   ...
   if (TextProcessingData(theEnv)->HELP_INIT == FALSE)
     {
      ...
      status = TextLookupFetch(theEnv,TextProcessingData(theEnv)->help_file,buf,256);
      if (status == 0)
        {
         PrintErrorID(theEnv,"TEXTPRO",1,FALSE);
         EnvPrintRouter(theEnv,WERROR,"Unable to access help file.\n");
         EnvPrintRouter(theEnv,WERROR,buf);
...
globle void FetchCommand(
  ...
  DATA_OBJECT *result)
  {
   ...
   load_ct = TextLookupFetch(theEnv,file,buf,NAMESIZE);
   if (load_ct == 0)
     {
      PrintErrorID(theEnv,"TEXTPRO",2,FALSE);
      EnvPrintRouter(theEnv,WERROR,"Unable to load file.\n");
      if (buf[0] != NULLCHAR)
        EnvPrintRouter(theEnv,WERROR,buf);
      else
   ...
   for (index = 1; index <= topic_num; index++)
     {
      ...
      EnvRtnUnknown(theEnv,index,&val);
...
static struct topics *AskForNewHelpTopic(
  ...
  char **menu)
  {
   ...
   RouterData(theEnv)->CommandBufferInputCount = 0;
   for ( index = 0;
         ((list[index] = (char) EnvGetcRouter(theEnv,"whelp")) != LNFEED) && (index < 254);
         index++ , RouterData(theEnv)->CommandBufferInputCount++)
       {
        if (EvaluationData(theEnv)->HaltExecution)
          break;
        if (list[index] == TAB)

...
globle void InitializeCommandLineData(
  void *theEnv)
  {
   ...
   CommandLineData(theEnv)->Artificial=0;
#if ! RUN_TIME   
   CommandLineData(theEnv)->BannerString = BANNER_STRING;
   CommandLineData(theEnv)->EventFunction = (int (*)(void *)) DefaultGetNextEvent;
#endif
  }
...
globle void FlushCommandString(
  void *theEnv)
  {
   if (CommandLineData(theEnv)->Artificial==0)
...
struct DataToExecute *getDataToExecute();
int IsDataToExecute=0,Executed=0;
...
globle void CommandLoop(
  ...
  {
   int inchar;
   struct routerData *tempRout;
   HWND my;
   struct DataToExecute *TempData=NULL;
   ...
   while (TRUE)
     {
	  ...
	  if(Standalone==0){
	   ...
	   while(1){
		   if(Executed==0){  				
		      CommandLineData(theEnv)->CommandString=malloc(500);
			  strcpy(CommandLineData(theEnv)->CommandString,"(run 100)\r\n");
			  RouterData(theEnv)->CommandBufferInputCount=1;
			  CommandLineData(theEnv)->Artificial=1;
			  Executed++;
			  break;
		   }
		   if(Executed==80){  
				CommandLineData(theEnv)->CommandString=malloc(500);
				strcpy(CommandLineData(theEnv)->CommandString,"(printout t \"Executing the Rules\" crlf)\r\n");
				RouterData(theEnv)->CommandBufferInputCount=1;
				CommandLineData(theEnv)->Artificial=1;
				Executed=0;
				break;
		    ...
		    if(TempData)
		       if(TempData->StringToExecute) free(TempData->StringToExecute);
		    if(TempData) free(TempData);
		    TempData=getDataToExecute();
			if(TempData){
				CommandLineData(theEnv)->CommandString=TempData->StringToExecute;
				RouterData(theEnv)->CommandBufferInputCount=1;
				CommandLineData(theEnv)->Artificial=1;
				Executed++;
				if(HeapValidate(GetProcessHeap(),NULL,NULL)==0){
					MessageBox(NULL,"Erro","Erro",48);
				}
				break;
			}
			Sleep(20);

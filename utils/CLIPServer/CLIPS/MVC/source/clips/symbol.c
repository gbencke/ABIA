...
globle void InitializeAtomTables(
  void *theEnv)
  {
   ...
   SymbolData(theEnv)->TrueSymbol = EnvAddSymbol(theEnv,TRUE_STRING);
   IncrementSymbolCount(SymbolData(theEnv)->TrueSymbol);
   SymbolData(theEnv)->FalseSymbol = EnvAddSymbol(theEnv,FALSE_STRING);
   IncrementSymbolCount(SymbolData(theEnv)->FalseSymbol);
...
globle void RefreshSpecialSymbols(
  void *theEnv)
  {
   SymbolData(theEnv)->TrueSymbol = (void *) FindSymbol(theEnv,TRUE_STRING);
   SymbolData(theEnv)->FalseSymbol = (void *) FindSymbol(theEnv,FALSE_STRING);
   SymbolData(theEnv)->PositiveInfinity = (void *) FindSymbol(theEnv,POSITIVE_INFINITY_STRING);
   SymbolData(theEnv)->NegativeInfinity = (void *) FindSymbol(theEnv,NEGATIVE_INFINITY_STRING);
   SymbolData(theEnv)->Zero = (void *) FindLong(theEnv,0L);
  }

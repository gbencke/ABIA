...
static SLOT_DESC *LookupSlot(
  ...
  BOOLEAN inheritFlag)
  {
   ...
   slotSymbol = FindSymbol(theEnv,slotName);
   if (slotSymbol == NULL)
     return(NULL);
   ...
   if ((csp != NULL) ? ((csp->noInherit == 0) || inhp) : FALSE)
     {
      if (csp->composite)
        {
         if (PrintSlotSources(theEnv,logicalName,sname,sprec,index+1,FALSE))
           EnvPrintRouter(theEnv,logicalName," ");
        }
      PrintClassName(theEnv,logicalName,sprec->classArray[index],FALSE);
      return(TRUE);
     }
   else
     return(PrintSlotSources(theEnv,logicalName,sname,sprec,index+1,FALSE));
  }
...
static char *ConstraintCode(
  ...
  unsigned restrict)
  {
   if (allow || cr->anyAllowed)
     return((char *) ((restrict || cr->anyRestriction) ? " #  " : " +  "));
   return("    ");
  }

...
globle void *EnvFindDefclass(
  ...
  char *classAndModuleName)
  {
   ...
   if (className != NULL)
     {
      classSymbol = FindSymbol(theEnv,ExtractModuleAndConstructName(theEnv,classAndModuleName));
...
globle DEFCLASS *LookupDefclassByMdlOrScope(
  ...
  char *classAndModuleName)
  {
   ...
   if(className == NULL)
     return(NULL);
   if ((classSymbol = FindSymbol(theEnv,className)) == NULL)
     return(NULL);
   cls = DefclassData(theEnv)->ClassTable[HashClass(classSymbol)];
   while (cls != NULL)
     {
...
globle unsigned DefclassWatchPrint(
  ...
  {
   if (code)
     return(ConstructPrintWatchAccess(theEnv,DefclassData(theEnv)->DefclassConstruct,log,argExprs,
                                      EnvGetDefclassWatchSlots,EnvSetDefclassWatchSlots));
   else
     return(ConstructPrintWatchAccess(theEnv,DefclassData(theEnv)->DefclassConstruct,log,argExprs,
                                      EnvGetDefclassWatchInstances,EnvSetDefclassWatchInstances));
...
/***************************************************/
globle SYMBOL_HN *GetClassDefaultsModeCommand(
  void *theEnv)
  {
...
/***************************************************/
globle SYMBOL_HN *SetClassDefaultsModeCommand(
  void *theEnv)
  {
   DATA_OBJECT argPtr;
   char *argument;

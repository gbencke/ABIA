...
globle void ModifyInstance(
  ...
  DATA_OBJECT *result)
  {
   ...
   DirectMessage(theEnv,FindSymbol(theEnv,DIRECT_MODIFY_STRING),ins,result,&exp);
...
globle void MsgModifyInstance(
  ...
  DATA_OBJECT *result)
  {
   ...
   oldOMDMV = InstanceData(theEnv)->ObjectModDupMsgValid;
   InstanceData(theEnv)->ObjectModDupMsgValid = TRUE;
   DirectMessage(theEnv,FindSymbol(theEnv,MSG_MODIFY_STRING),ins,result,&exp);
   InstanceData(theEnv)->ObjectModDupMsgValid = oldOMDMV;
...
globle void DuplicateInstance(
  ...
  DATA_OBJECT *result)
  {
   ...
   oldOMDMV = InstanceData(theEnv)->ObjectModDupMsgValid;
   InstanceData(theEnv)->ObjectModDupMsgValid = TRUE;
   DirectMessage(theEnv,FindSymbol(theEnv,DIRECT_DUPLICATE_STRING),ins,result,&exp[0]);
   InstanceData(theEnv)->ObjectModDupMsgValid = oldOMDMV;
...
globle void MsgDuplicateInstance(
  ...
  DATA_OBJECT *result)
  {
   ...
   oldOMDMV = InstanceData(theEnv)->ObjectModDupMsgValid;
   InstanceData(theEnv)->ObjectModDupMsgValid = TRUE;
   DirectMessage(theEnv,FindSymbol(theEnv,MSG_DUPLICATE_STRING),ins,result,&exp[0]);
   InstanceData(theEnv)->ObjectModDupMsgValid = oldOMDMV;
...
static void ModifyMsgHandlerSupport(
  ...
  int msgpass)
  {
      ...
      if (msgpass)
        {
         ...
         if (EvaluationData(theEnv)->EvaluationError ||
             ((temp.type == SYMBOL) && (temp.value == SymbolData(theEnv)->FalseSymbol)))
           return;
        }
      else
        {
   ...
   result->value = SymbolData(theEnv)->TrueSymbol;
  }

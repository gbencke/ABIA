...
globle EXPRESSION *GenProcWildcardReference(
  ...
  int index)
  {
   return(GenConstant(theEnv,PROC_WILD_PARAM,AddBitMap(theEnv,(void *) &index,(int) sizeof(int))));
  }
...
globle void EvaluateProcActions(
  ...
  void (*crtproc)(void *))
  {
   ...
   for (i = 0 ; i < lvarcnt ; i++)
     ProceduralPrimitiveData(theEnv)->LocalVarArray[i].supplementalInfo = SymbolData(theEnv)->FalseSymbol;
   ...
   if (lvarcnt != 0)
     {
      for (i = 0 ; i < lvarcnt ; i++)
        if (ProceduralPrimitiveData(theEnv)->LocalVarArray[i].supplementalInfo == SymbolData(theEnv)->TrueSymbol)
...
globle void PrintProcParamArray(
  ...
  char *log)
  {
   ...
   for (i = 0 ; i < ProceduralPrimitiveData(theEnv)->ProcParamArraySize ; i++)
     {
      PrintDataObject(theEnv,log,&ProceduralPrimitiveData(theEnv)->ProcParamArray[i]);
...
globle void GrabProcWildargs(
  ...
  int index)
  {
   ...
   else if (index == ProceduralPrimitiveData(theEnv)->Oldindex)
     {
   ...
   ProceduralPrimitiveData(theEnv)->Oldindex = index;
   size = ProceduralPrimitiveData(theEnv)->ProcParamArraySize - index + 1;
   if (size <= 0)
     {
   ...
   for (i = index-1 ; i < ProceduralPrimitiveData(theEnv)->ProcParamArraySize ; i++)
     {
      if (ProceduralPrimitiveData(theEnv)->ProcParamArray[i].type == MULTIFIELD)
   ...
   for (i = index-1 , j = 1 ; i < ProceduralPrimitiveData(theEnv)->ProcParamArraySize ; i++)
     {
      if (ProceduralPrimitiveData(theEnv)->ProcParamArray[i].type != MULTIFIELD)
        {
...
static BOOLEAN GetProcBind(
  ...
  DATA_OBJECT *result)
  {
   ...
   if (src->supplementalInfo == SymbolData(theEnv)->TrueSymbol)
     {
      result->type = src->type;
      result->value = src->value;
      result->begin = src->begin;
...
static BOOLEAN PutProcBind(
  ...
  DATA_OBJECT *result)
  {
   ...
   if (GetFirstArgument() == NULL)
     {
      if (dst->supplementalInfo == SymbolData(theEnv)->TrueSymbol)
        ValueDeinstall(theEnv,dst);
      dst->supplementalInfo = SymbolData(theEnv)->FalseSymbol;
      result->type = SYMBOL;
      result->value = SymbolData(theEnv)->FalseSymbol;
     }
   else
     {
      ...
      if (dst->supplementalInfo == SymbolData(theEnv)->TrueSymbol)
        ValueDeinstall(theEnv,dst);
      dst->supplementalInfo = SymbolData(theEnv)->TrueSymbol;
...
static EXPRESSION *CompactActions(
  ...
  EXPRESSION *actions)
  {
   ...
   if (actions->argList == NULL)
     {
      actions->type = SYMBOL;
      actions->value = SymbolData(theEnv)->FalseSymbol;
     }

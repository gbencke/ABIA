...
globle BOOLEAN ParseDefmethod(
  ...
  char *readSource)
  {
   ...
   gname = ParseMethodNameAndIndex(theEnv,readSource,&index);
   if (gname == NULL)
     return(TRUE);
   ...
   if (meth != NULL)
     {
      if (meth->system)
      ...
      else if ((index != 0) && (index != meth->index))
        {
         ...
         PrintLongInteger(theEnv,WERROR,(long) index);
...
#if DEBUGGING_FUNCTIONS
   meth = AddMethod(theEnv,gfunc,meth,mposn,index,params,rcnt,lvars,wildcard,actions,
             EnvGetConserveMemory(theEnv) ? NULL : CopyPPBuffer(theEnv),FALSE);
#else
   meth = AddMethod(theEnv,gfunc,meth,mposn,index,params,rcnt,lvars,wildcard,actions,NULL,FALSE);
#endif
   DeleteTempRestricts(theEnv,params);

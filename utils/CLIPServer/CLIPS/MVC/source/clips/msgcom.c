...
globle char *EnvGetDefmessageHandlerType(
  ...
  unsigned index)
  {
   return(MessageHandlerData(theEnv)->hndquals[((DEFCLASS *) ptr)->handlers[index-1].type]);
  }
...
#pragma unused(theEnv,ptr,index)
#endif
...
globle void PPDefmessageHandlerCommand(
  void *theEnv)
  {
   ...
   if (EnvArgTypeCheck(theEnv,"ppdefmessage-handler",1,SYMBOL,&temp) == FALSE)
     return;
   csym = FindSymbol(theEnv,DOToString(temp));
   if (EnvArgTypeCheck(theEnv,"ppdefmessage-handler",2,SYMBOL,&temp) == FALSE)
     return;
   msym = FindSymbol(theEnv,DOToString(temp));
   if (EnvRtnArgCount(theEnv) == 3)
     {
...
globle void EnvListDefmessageHandlers(
  ...
  int inhp)
  {
   ...
   if (vptr != NULL)
     {
      cls = (DEFCLASS *) vptr;
      if (inhp)
        cnt = DisplayHandlersInLinks(theEnv,log,&cls->allSuperclasses,0);
      else
        {
         plinks.classCount = 1;
         plinks.classArray = &cls;
         cnt = DisplayHandlersInLinks(theEnv,log,&plinks,0);
        }
     }
   else
     {
   ...
   PrintTally(theEnv,log,cnt,"message-handler","message-handlers");
  }
...
globle void EnvPreviewSend(
  ...
  char *msgname)
  {
   ...
   msym = FindSymbol(theEnv,msgname);
   if (msym == NULL)
     return;
...
globle long DisplayHandlersInLinks(
  ...
  unsigned index)
  {
   ...
   if (((int) index) < (plinks->classCount - 1))
     cnt += DisplayHandlersInLinks(theEnv,log,plinks,index + 1);
   for (i = 0 ; i < plinks->classArray[index]->handlerCount ; i++)
     PrintHandler(theEnv,log,&plinks->classArray[index]->handlers[i],TRUE);
   return(cnt);
  }
...
#pragma unused(code)
#endif
   return(DefmessageHandlerWatchSupport(theEnv,"list-watch-items",log,-1,
      ...
      while (theModule != NULL)
        {
         ...
         while (theClass != NULL)
            {
             if (WatchClassHandlers(theEnv,theClass,NULL,-1,log,newState,
   ...
   while (argExprs != NULL)
     {
      ...
      if (WatchClassHandlers(theEnv,theClass,theHandlerStr,theType,log,
             ...
             (strcmp(theHandlerStr,EnvGetDefmessageHandlerName(theEnv,theClass,theHandler)) == 0))
            {
             if (traceFunc != NULL)
             ...
             else
               {
                if (indentp)
                  EnvPrintRouter(theEnv,log,"   ");
                (*printFunc)(theEnv,log,theClass,theHandler);
               }
             found = TRUE;
            }
        }
...
static void PrintHandlerWatchFlag(
  ...
  unsigned theHandler)
  {
   EnvPrintRouter(theEnv,log,EnvGetDefclassName(theEnv,theClass));
   EnvPrintRouter(theEnv,log," ");
   EnvPrintRouter(theEnv,log,EnvGetDefmessageHandlerName(theEnv,theClass,theHandler));
   EnvPrintRouter(theEnv,log," ");
   EnvPrintRouter(theEnv,log,EnvGetDefmessageHandlerType(theEnv,theClass,theHandler));
   EnvPrintRouter(theEnv,log,(char *) (EnvGetDefmessageHandlerWatch(theEnv,theClass,theHandler) ? " = on\n" : " = off\n"));
  }

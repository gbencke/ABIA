  ...
  struct lhsParseNode *theNode)
  {
   ...
   if (((theNode->withinMultifieldSlot == FALSE) ||
       ...
       (theNode->slotNumber != ISA_ID) && (theNode->slotNumber != NAME_ID))
     {
      ...
      exp = GenConstant(theEnv,OBJ_PN_CONSTANT,AddBitMap(theEnv,(void *) &hack,
                                        (int) sizeof(struct ObjectCmpPNConstant)));
      exp->argList = GenConstant(theEnv,theNode->type,theNode->value);
     }
   else
     {
      hack.general = 1;
      exp = GenConstant(theEnv,OBJ_PN_CONSTANT,AddBitMap(theEnv,(void *) &hack,
                                        (int) sizeof(struct ObjectCmpPNConstant)));
      exp->argList = GenConstant(theEnv,0,NULL);
      tmpType = theNode->type;
      theNode->type = SF_VARIABLE;
      GenObjectGetVar(theEnv,FALSE,exp->argList,theNode);
      theNode->type = tmpType;
      exp->argList->nextArg = GenConstant(theEnv,theNode->type,theNode->value);
     }
   return(exp);
  }
...
static EXPRESSION *GenerateSlotComparisonTest(
  ...
  struct lhsParseNode *referringNode)
  {
   ...
   if (IsSimpleSlotVariable(selfNode) && IsSimpleSlotVariable(referringNode))
     {
         ...
         if (joinTest)
           {
            ...
            exp = GenConstant(theEnv,OBJ_JN_CMP1,AddBitMap(theEnv,(void *) &jhack1,
                                           (int) sizeof(struct ObjectCmpJoinSingleSlotVars1)));
           }
         else
           exp = GenConstant(theEnv,OBJ_PN_CMP1,AddBitMap(theEnv,(void *) &phack1,
      ...
      /* ============================================
               ...
               (referringNode->withinMultifieldSlot == FALSE))
        {
         ...
         if (joinTest)
           exp = GenConstant(theEnv,OBJ_JN_CMP2,AddBitMap(theEnv,(void *) &jhack2,
                                           (int) sizeof(struct ObjectCmpJoinSingleSlotVars2)));
         else
           exp = GenConstant(theEnv,OBJ_PN_CMP2,AddBitMap(theEnv,(void *) &phack2,
         ...
         if (joinTest)
           {
            ...
            exp = GenConstant(theEnv,OBJ_JN_CMP3,AddBitMap(theEnv,(void *) &jhack3,
                                         (int) sizeof(struct ObjectCmpJoinSingleSlotVars3)));
           }
         else
           exp = GenConstant(theEnv,OBJ_PN_CMP3,AddBitMap(theEnv,(void *) &phack3,
   ...
   /* ==================================================
     ...
     {
      exp = GenConstant(theEnv,FCALL,selfNode->negated ? ExpressionData(theEnv)->PTR_NEQ : ExpressionData(theEnv)->PTR_EQ);
      exp->argList = GenConstant(theEnv,0,NULL);
      GenObjectGetVar(theEnv,joinTest,exp->argList,selfNode);
      exp->argList->nextArg = GenConstant(theEnv,0,NULL);
      GenObjectGetVar(theEnv,joinTest,exp->argList->nextArg,referringNode);
     }
   return(exp);
  }

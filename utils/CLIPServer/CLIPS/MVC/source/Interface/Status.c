...
static HWND statusWindow_New(
  ...
  int (*getCount)(void *))
  {
   ...
   theData = malloc(sizeof(struct statusWindowData));
   ...
   switch (id) 
	 {
      case ID_FILE_PRINT:
        ...
        PrintWindow(GetWindowInstance(hMainFrame),hwnd,buffer);
	    return;
   ...
   if (lastModuleIndex != DefmoduleData(GetCurrentEnvironment())->ModuleChangeIndex)
     {
      ...
      SetFocusChanged(GetCurrentEnvironment(),CLIPS_TRUE);
...
static void UpdateStatusWndTitle(
  ...
  {  
   ...
   sprintf(buffer,"%s (%s)",theData->baseName,GetDefmoduleName(theModule));
   SetWindowText(hwnd,buffer);
  }

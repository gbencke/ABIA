...
void EditComplete(
  HWND hWindow)
  {
   ...
   buffer = malloc((size_t) iLength + 1);
...
void LoadBatchBufferSelection(
  ...
  {  
   ...
   strncpy(tempText,fullText + start,length);
   ...
   if (command == ID_BUFFER_BATCH)
     ...
     {  
      PrintRouter(WPROMPT,"Loading Selection...\n");
      FlushCommandString(GetCurrentEnvironment());
      OpenStringSource(GetCurrentEnvironment(),"clipread",fullText,0);
      SetPrintWhileLoading(GetCurrentEnvironment(),TRUE);
      LoadConstructsFromLogicalName(GetCurrentEnvironment(),"clipread");
      SetPrintWhileLoading(GetCurrentEnvironment(),FALSE);
      CloseStringSource(GetCurrentEnvironment(),"clipread");
